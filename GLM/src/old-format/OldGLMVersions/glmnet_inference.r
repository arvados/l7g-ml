...
w3 <- 1/abs(matrix(coef(cv.ridge, s=cv.ridge$lambda.min)
                   [, 1][2:(ncol(Xmat)+1)] ))^2 ## Using gamma = .5
...
lambda = lmanda.min*length(y)

#aa = fixedLassoInf(Xmat,y,coefVec,lambda,intercept=F,family='binomial',standardize=F)

#pvals[which(beta != 0)] <- aa$pv
#ci[which(beta != 0)] <- aa$ci

#dataF_ps <- data.frame("pvals" = pvals, "ci" = ci)
#filename <- paste0('glmnet_lasso_',colorblood,'_',type_measure,'ps.txt' )
#fileConn <- file(filename, "w")
#write.table(dataF_ps, fileConn, sep= "\t", row.names = FALSE)
#close(fileConn)
