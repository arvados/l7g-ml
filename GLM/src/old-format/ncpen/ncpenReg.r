...
cv_fit <- cv.ncpen(y,Xmat,family='binomial',penalty='scad',alpha=1,x.standardize=FALSE,intercept=TRUE)
...
coefVec <- coef(cv_fit,which=cv_fit$lambda.which)
coefVec <- coefVec[-1]
idxnzmin <- which(coefVec !=0)
sizeCoefMin <- length(coef(cv_fit,which=cv_fit$lambda.which))
